...
for( int j0 = 0; j0 < n0; j0++) {
    ...
    { // q0=3
      BEGIN_INSTRUMENTATION; // loop:q0
      for( int r0 = 0; r0 < R; r0++) {
        int   w_qr_3_offset_3 = 3*(R);
                    int   w_qr_3_idx_3    = w_qr_3_offset_3 + r0;
                    float *w_qr_3_addr_3  = &weights[w_qr_3_idx_3];
                    float w_qr_3        = *w_qr_3_addr_3;
        
                    int    iq_shift_3    = 3 + i0;
                    int    iq_row_wrap_3 = iq_shift_3 % m0;
                    int    jr_shift_3    = r0 + j0;
                    int    jr_col_wrap_3 = jr_shift_3 % n0;
                    int    x_iqjr_3_idx_3  = iq_row_wrap_3 * n0 + jr_col_wrap_3;
                    float *x_iqjr_3_addr_3 = &x[x_iqjr_3_idx_3];
                    float  x_iqjr_3      = *x_iqjr_3_addr_3;
        
                    int    y_ij_idx    = i0*n0 + j0;
                    float *y_ij_addr   = &y[y_ij_idx];
                
                    float y_ij      = *y_ij_addr;
                    float res_wx_3    = w_qr_3 * x_iqjr_3;
                    float acc_y_ij_3  = y_ij + res_wx_3;
                    *y_ij_addr      = acc_y_ij_3;
      }
      END_INSTRUMENTATION; // loop:q0
    }
