...
def main():
  ...
  operation = open("../operations/" + operation_file, 'r')
  schedule = open("../schedules/" + schedule_file, 'r')
  ...
  schedule_lines = schedule.readlines()
  split_vars = {}
  for schedule_line in schedule_lines:
      schedule_arr = [s for s in schedule_line.strip().split(" ") if s]
      schedule_spec = schedule_arr[0]
      if schedule_spec == "interchange":
        swap_index_1 = schedule_arr[1]
        swap_index_2 = schedule_arr[2]
        swap_order_1 = loop_order.index(swap_index_1)
        swap_order_2 = loop_order.index(swap_index_2)
        loop_order[swap_order_1] = swap_index_2
        loop_order[swap_order_2] = swap_index_1
      elif schedule_spec == "split":
        split_vars[schedule_arr[1]] = [schedule_arr[1], schedule_arr[3], schedule_arr[4]]
      else:
          print("Unroll not implemented")
  ...
  out = open(f"versions/{output_file_name}", "w")
  out.write(code)
  out.close()
