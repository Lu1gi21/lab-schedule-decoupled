...
extern void COMPUTE_NAME( int m0, int n0,
			      float *input_x,
			      float *output_y );

extern double COMPUTE_FLOP_NAME( int m0, int n0 );
extern double COMPUTE_BYTES_NAME( int m0, int n0 );
...
int main( int argc, char *argv[] )
{
  ...
  for( int p = min_size;
       ...
       p += step_size )
    {
      ...
      double flop   =  COMPUTE_FLOP_NAME( m0, n0 );
      double throughput   =  flop / nanoseconds;
      double bytes        =  COMPUTE_BYTES_NAME( m0, n0 );
      double gbytes_per_s =  COMPUTE_BYTES_NAME( m0, n0 )/ nanoseconds;
