/*
  This is the SSA version of the unrolled q loop implementation.
...
#ifndef COMPUTE_NAME
#define COMPUTE_NAME ssa_unroll_q
#endif

#ifndef COMPUTE_FLOP_NAME
#define COMPUTE_FLOP_NAME ssa_unroll_q_flop
#endif

#ifndef COMPUTE_BYTES_NAME
#define COMPUTE_BYTES_NAME ssa_unroll_q_bytes
#endif
    ...
    for(int j0 = 0; j0 < n0; ++j0) {
      ...
      // q0 = 1
      {
        for(int r0 = 0; r0 < (R); ++r0) {
          ...
          // Compute weight access for q0=1
          int w_qr_offset = 1*(R);
          int w_qr_idx = w_qr_offset + r0;
          float *w_qr_addr = &weights[w_qr_idx];
          float w_qr = *w_qr_addr;
          
          // Compute x array access for q0=1
          int iq_shift = 1 + i0;
          int iq_row_wrap = iq_shift % m0;
          int jr_shift = r0 + j0;
          int jr_col_wrap = jr_shift % n0;
      ...
      // q0 = 2
      {
        for(int r0 = 0; r0 < (R); ++r0) {
          ...
          // Compute weight access for q0=2
          int w_qr_offset = 2*(R);
          int w_qr_idx = w_qr_offset + r0;
          float *w_qr_addr = &weights[w_qr_idx];
          float w_qr = *w_qr_addr;
          
          // Compute x array access for q0=2
          int iq_shift = 2 + i0;
          int iq_row_wrap = iq_shift % m0;
          int jr_shift = r0 + j0;
          int jr_col_wrap = jr_shift % n0;
